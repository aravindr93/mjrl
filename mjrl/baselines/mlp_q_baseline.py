...
class MLPQBaseline(MLPBaseline):
    def __init__(self, env_spec, inp_dim=None, inp='obs', learn_rate=1e-3, reg_coef=0.0,
        ...
        self.n = inp_dim if inp_dim is not None else (env_spec.observation_dim + env_spec.action_dim)
        ...
        layer_sizes = (self.n, ) + hidden_sizes + (1, )
    ...
    def _features(self, o, a):
        o = np.clip(o, -10, 10)/10.0
        if o.ndim > 2:
            o = o.reshape(o.shape[0], -1)
        feat_mat = np.concatenate([o, a], axis=-1)
        return feat_mat

    def fit(self, o, a, returns, return_errors=False):
        featmat = self._features(o, a)
    ...
    def predict(self, o, a):
        featmat = self._features(o, a).astype('float32')
        if self.use_gpu:
